...
The pipeline generates trained models (`.keras` & `.h5`) and embedding vectors capable of:
...
│
├── models/                   ← Saved model architectures (small files only)
├── embeddings/               ← CSV embedding vectors (small samples)
├── outputs/                  ← Plots, summary CSVs (e.g., model_results.csv)
└── large_files/              ← (Not on GitHub) Location for large .h5/.bin models
